...
namespace editor {
     ...
     */
    void DisplayMetadata::loadFileAsThumbnail(const fs::path& inFilePath) {
        try {
            Buffer buffer = DataReader::readFile(inFilePath); // takes ownership hopefully
            if (DataReader reader(buffer.span(), Endian::Big); isPngHeader(reader)) {
                thumbnail = std::move(buffer);
            }
        } catch (const std::runtime_error& e) {
            cmn::log(cmn::eLog::error, "Failed to load default thumbnail, will be black instead.");
        };
    }
            ...
            if (chunkType == "tEXt") {
                ...
                while (reader.tell() < endOfChunk) {
                    ...
                    if (key == "4J_SEED") {
                        ...
                        extraData = text; // stringToHex(text);
    ...
    Buffer DisplayMetadata::write(lce::CONSOLE c) {
        ...
        DataWriter writer(16348, Endian::Big);
    ...
    void DisplayMetadata::writePNG(DataWriter& writer, lce::CONSOLE c) {
        const Endian prevEndian = writer.getEndian();
        ...
        put("4J_EXTRADATA");   nul(); put(extraData);                nul(); // put(hexToString(extraData));
