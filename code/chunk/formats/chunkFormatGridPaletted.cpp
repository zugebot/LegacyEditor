...
namespace editor {
    ...
    void ChunkFormatGridPaletted::readBlocks(std::span<const u8> dataIn, u8* oldBlockPtr) {
        ...
        DataReader gridReader(gridHeader, Endian::Big);
        ...
        for (i32 gridY = 0; gridY < 32; gridY++) {
            ...
            MU u16 gridData = gridReader.read<u16>();
    ...
    bool ChunkFormatGridPaletted::readGrid(u8 const* gridDataPtr, u8 blockBuffer[MAX_BLOCKS_SIZE]) {
    ...
    void ChunkFormatGridPaletted::writeChunkInternal(ChunkData* chunkData, WriteSettings& settings, DataWriter& writer, bool fastMode) {
        writer.write<i32>(chunkData->chunkX);
        writer.write<i32>(chunkData->chunkZ);
        writer.write<i64>(chunkData->lastUpdate);

        if (settings.m_schematic.chunk_lastVersion > 8) {
        ...
        writeBlocks(writer, settings, &oldBlocks[0], true);
        writeBlocks(writer, settings, &oldBlocks[32768], false);
    ...
    MU void ChunkFormatGridPaletted::writeBlocks(DataWriter& writer, MU WriteSettings& settings, u8 const* oldBlockPtr, bool isBottom) {
    ...
    template<size_t BitsPerBlock>
    void ChunkFormatGridPaletted::writeGrid(DataWriter& writer,
