...
namespace editor {
    ...
    namespace scoring {
        ...
        template<typename T>
        inline u8 get(editor::chunk::eBlockOrder O, const std::vector<T>& a, int x, int y, int z) {
            return a[toIndex(O, x, y, z)];
        }

        template<typename T, int Bits, bool ShouldBitBeSet, editor::chunk::eBlockOrder Order>
        Score scoreChunk(const std::vector<T>& a) {
            const bool isHalf = (Order == editor::chunk::eBlockOrder::yXZ); // only 0-127 encoded
        ...
        MU editor::chunk::eBlockOrder guessOrder(const std::vector<T>& data,
