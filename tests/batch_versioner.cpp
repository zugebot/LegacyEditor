...
eBlockOrder guessOrder(const std::vector<T>& data, const std::vector<u8>& heightMap) {
    // int size = data.size() * bits / 8;
    c_u64 tinier = 32768 / bits * 8; // Y=128
    c_u64 bigger = 65536 / bits * 8; // Y=256
    ...
    if (data.size() == tinier) cand.emplace_back(eBlockOrder::yXZ, scoreChunk<T, bits, eBlockOrder::yXZ>(data));
    if (data.size() == bigger) cand.emplace_back(eBlockOrder::yXZy, scoreChunk<T, bits, eBlockOrder::yXZy>(data));
...
int main(MU int argc, char* argv[]) {
    ...
    for (auto [tuLabel, thumbnail, savegame]: versionFiles) {
        ...
        if (fileListing.read(savegame.string()) != 0) {
            std::cerr << "Failed to load file: " << savegame.make_preferred() << "\n";
            continue;
        } else {
            std::cout << tuLabel << ": " /*<< savegame.make_preferred()*/ << "\n";

            // 1. fileListing.myReadSettings.myOldestVersion
            int oldestVersion = fileListing.myReadSettings.getOldestVersion();
            std::cout << "fileListing.OldestVersion : " << oldestVersion << "\n";
            // 2. fileListing.myReadSettings.myCurrentVersion
            int currentVersion = fileListing.myReadSettings.getCurrentVersion();
            ...
            for (auto regionFile: fileListing.ptrs.region_overworld) {
            ...
            // 3. ChunkManager.anon.isCompressed
            // std::cout << "ChunkManager Version: " << chunkManager->fileData.getCompressedFlag() << "\n";

            // 4. ChunkManager.anon.rleFlag
            // std::cout << "ChunkManager rleFlag: " << chunkManager->fileData.getRLEFlag() << "\n";

            // 5. ChunkManager.anon.newSaveFlag
            // std::cout << "ChunkManager newSaveFlag: " << chunkManager->fileData.getNewSaveFlag() << "\n";
